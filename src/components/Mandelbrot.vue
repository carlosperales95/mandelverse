<template>
  <div class="relative w-full h-screen overflow-hidden">
    ...
    <!-- Info Badge -->
    <div
      class="fixed top-4 left-4 z-40 bg-black/50 backdrop-blur-sm text-white px-4 py-2 rounded-lg text-sm shadow-lg"
    >
...
const checkFrameSimilarity = () => {
  ...
  if (previousFrameData) {
    ...
    if (sim >= similarityThreshold.value) {
      sameFrameCount++;
      if (sameFrameCount >= maxSameFrames.value) skipToNextPoint();
